...
- `plotStarSpell(address _starGuard, address _starSpell, bytes32 _starSpellTag)`: Whitelist a star spell on its star guard so it can be executed permissionlessly. `_starSpellTag` is the expected codehash of the star spell.
- `dropStarSpell(address _starGuard)`: Remove the currently whitelisted star spell from a star guard.
